...
const handleSendMessage = async () => {
  try {
    ...
    const res = await axios.post("http://localhost:4002/bot/message", {
      text: input,
    });
    if (res.status === 200) {
      setmessage([
    ...
    console.log("error in the sending messages");
  }
  setinput("");
  setloading(false);
};
   ...
   <div className='flex flex-col min-h-screen bg-[#0d0d0d] text-white'>
      ...
      <header className="fixed top-0 left-0 w-full border-b border-gray-800 bg-[#0d0d0d] z-10">
        <div className=" container mx-auto flex justify-between items-center px-6 py-4">
          <h1 className="text-lg font-bold">Future-guide chatbot</h1>
